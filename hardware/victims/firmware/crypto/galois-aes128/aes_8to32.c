/* File: "aes.c". Automatically generated by SBV from Haskell. Modified to implement on MCU. */
...
void GALOIS_AES128_setkey(const uint8_t *key)
{
	/*
	key[4] = {
		0x2b731516, 0x28aed2a6, 0xabf71588, 0x09cf4f3c
	};
	key = DEFAULT_KEY;   
	*/
}
...
//                        SWord32*ct)
void GALOIS_AES128_blockencrypt(const uint8_t *pt, const uint8_t *key,
                        uint8_t *ct)
{
  const SWord32 s0 = pt[0] | (pt[1]<<8) | (pt[2]<<16) | (pt[3]<<24);
  const SWord32 s1 = pt[4] | (pt[5]<<8) | (pt[6]<<16) | (pt[7]<<24);
  const SWord32 s2 = pt[8] | (pt[9]<<8) | (pt[10]<<16) | (pt[11]<<24);  
  const SWord32 s3 = pt[12] | (pt[13]<<8) | (pt[14]<<16) | (pt[15]<<24);
  const SWord32 s4 = key[0] | (key[1]<<8) | (key[2]<<16) | (key[3]<<24);
  const SWord32 s5 = key[4] | (key[5]<<8) | (key[6]<<16) | (key[7]<<24);
  const SWord32 s6 = key[8] | (key[9]<<8) | (key[10]<<16) | (key[11]<<24);
  const SWord32 s7 = key[12] | (key[13]<<8) | (key[14]<<16) | (key[15]<<24);
